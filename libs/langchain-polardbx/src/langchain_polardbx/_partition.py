"""Partition clause helpers — no dependency on sqlalchemy.

This module provides the shared partition-clause builder and identifier
validation utilities used by both ``PolarDBXVectorStore`` (core install)
and ``create_partitioned_table`` (``[sql]`` extra).

Keeping these functions in a standalone module ensures that VectorStore's
partition features work without sqlalchemy installed.
"""
